...
public class AsideRulePreferencePage extends PreferencePage implements
	...
	public boolean performOk() {
		...
		if (_asideRulesCheck.getSelection() != pre_asideRulesCheck) {
			Plugin.getDefault().setAsideTBCheckPreference(
					_asideRulesCheck.getSelection());
		}

		if (_projectRulesCheck.getSelection() != pre_projectRulesCheck) {
			Plugin.getDefault().setProjectTBCheckPreference(
		...
		if (_externalRulesCheck.getSelection() && pre_externalRulesCheck) {
			...
			if (differentFrom(current_items, pre_items)) {
				Plugin.getDefault().setTBPathPreference(
						_pathList.getItems());
			}
				...
				&& pre_externalRulesCheck) {
			Plugin.getDefault().setTBPathPreference(null);
			Plugin.getDefault().setExternalTBCheckPreference(false);
		} else if (_externalRulesCheck.getSelection()
				&& !pre_externalRulesCheck) {
			Plugin.getDefault().setExternalTBCheckPreference(true);
			Plugin.getDefault().setTBPathPreference(_pathList.getItems());
		}

		// Validation rules setting
		Plugin.getDefault().setAsideVRCheckPreference(
				asideRulesCheck.getSelection());

		Plugin.getDefault().setProjectVRCheckPreference(
				projectRulesCheck.getSelection());

		if (externalRulesCheck.getSelection()) {
			Plugin.getDefault().setVRPathPreference(pathList.getItems());
			Plugin.getDefault().setExternalVRCheckPreference(true);
		} else {
			Plugin.getDefault().setVRPathPreference(null);
			Plugin.getDefault().setExternalVRCheckPreference(false);
		}
