...
public class EncodingResolution implements IMarkerResolution,
	...
	public String getDescription() {
	    ...
	    if(fStrategyType.equals(PluginConstants.ENCODING_TYPES[0])){
			description = "Use CSS Encoding when the data you get from an outside source (i.e., the user) is to be written directly into your Cascading Style Sheet or your style tag. Doing so will encode (â€œcleanâ€�) malicious characters so an attacker can not easily exploit CSS interpreter vulnerabilities.";
			info = "&lt;style&gt; {property: ...DONâ€™T TRUST OUTSIDE DATA IN HERE...} &lt;/style&gt;";
		}else if(fStrategyType.equals(PluginConstants.ENCODING_TYPES[1])){
			description = "Use HTML Encoding when the data you get from an outside source (i.e., the user) is to be written directly into your HTML body. Characters / statements such as &lt;script&gt; will be cleansed to &amp;lt;script&amp;gt; which the browser then interprets as the word â€œ&lt;script&gt;â€� rather than the command to run a script.";
			info = "&lt;body&gt;...DONâ€™T TRUST OUTSIDE DATA IN HERE...&lt;/body&gt;";
		}else if(fStrategyType.equals(PluginConstants.ENCODING_TYPES[2])){
			description = "Attributes provide information about an element (e.g., align=â€�centerâ€�) and use a minimal set of characters. Use the HTML Attribute Encoder when attributes you use in your HTML do not come from your HTML page to safeguard from dangerous characters.";
			info = "&lt;div> attr=...DONâ€™T TRUST OUTSIDE DATA IN HERE...&gt;content&lt;/div&gt;";
			...
			info = "&lt;script&gt;alert(â€˜...DONâ€™T TRUST OUTSIDE DATA IN HERE...â€™)&lt;/script&gt;";
		}
	...
	public Image getImage() {
		return Plugin.getImageDescriptor("devil.png").createImage();
	}
