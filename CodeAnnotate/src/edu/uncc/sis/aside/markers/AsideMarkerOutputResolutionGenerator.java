...
public class AsideMarkerOutputResolutionGenerator implements
	...
	private boolean internalHasResolution(IMarker marker) {
		...
		try {
			...
			if (markerType == null
					|| !(markerType.equals(PluginConstants.MARKER_INPUT_VALIDATION)||markerType.equals(PluginConstants.MARKER_MULTI_OUTPUT_ENCODING))) {
				return false;
			}
	...
	private IMarkerResolution[] internalGetResolutions(IMarker marker) {
		...
		IProject project = MarkerAndAnnotationUtil.getProjectFromICompilationUnit(fCompilationUnit);
		  DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		    ...
		    logger.info(dateFormat.format(hoverDate) + " " + Plugin.getUserId() +" hovers over the marker or clicks on the marker of output encoding warning at Line "
